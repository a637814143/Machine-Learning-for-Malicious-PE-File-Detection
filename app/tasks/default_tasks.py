...
try:  # pragma: no cover - optional dependency for runtime usage
    lgb = import_lightgbm()
except ModuleNotFoundError as exc:  # pragma: no cover - mirror prediction module behaviour
...
def train_model_task(args, progress, text):
    ...
    if validation_vectors:
        ...
        text("未指定验证集，将使用默认配置或仅基于训练集监控指标")
    ...
    try:
        booster = train_ember_model_from_npy(
            npy_path,
            output_dir,
    ...
    summary_lines = [
        "模型训练完成。",
        f"模型文件目标: {_infer_model_output_path(output_dir)}",
    ]
    if validation_vectors:
        summary_lines.append(f"验证集: {validation_vectors}")
    else:
        summary_lines.append("验证集: 未指定（使用默认或跳过）")
    ...
    if isinstance(best_scores, dict) and best_scores:
        ...
        for dataset_name, metrics in best_scores.items():
            ...
            metric_lines.append(f"  - {dataset_name}: {formatted_metrics}")
        if metric_lines:
